...
{					// if user logged in
	if(isset($_SESSION['session_username'])) 
	{
		...
		if(mysqli_num_rows($result) == 1)  // if fetch is 1 result
		{
			...
			{					// query db to add like
				$query='update picture set likes = likes + 1 where picture_id="'. $picture_id .'"';
				$result=mysqli_query($dbc, $query) or trigger_error("Query: $query\n<br/> 
									MySQL Error: ". mysqli_error($dbc) );
				if(mysqli_affected_rows($dbc) == 1) // if like added
				{
					$_SESSION['id_rated'] = $picture_id; // do not like again in this session

					$likes = $photo_infos['likes'] +1;
					$dislikes = $photo_infos['dislikes'];
